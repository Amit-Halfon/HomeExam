...
def xor_encryption(text, key):
    ...
    for i in range(len(text)):
        xor_char = (ord(text[i]) ^ ord(key[i % 3]))
        encrypted.append(xor_char)
    return encrypted
...
def xor_decryption(encrypted_text,key):
    ...
    for i in range(len(encrypted_text)):
        xor_char=encrypted_text[i] ^ ord(key[i % 3])
        decrypted.append(chr(xor_char))
...
def findMaxCandidate(group):
    # was told to not take " " as the most common char
    ...
    for k in range(32, 128):
        ...
        for c in group:
            candidate=k^c
            if chr(candidate).upper() in letterFrequency:
                score+=letterFrequency[chr(candidate).upper()]
        if score>max_score:
            max_score=score
            max_cand=chr(k)
    return max_cand
